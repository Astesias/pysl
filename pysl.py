...
class ezlog():
    def __init__(self,filename):
        logging.basicConfig(
                            filename=filename,
                            format='%(asctime)s : %(message)s', # %(module)s.py line %(lineno)d
                            datefmt='%Y/%m/%d|%H:%M:%S',
                            level=logging.INFO
                            )
...
class excel_access:
    ...
    def write_cell(self,x,y,obj):
        ...
        self.table.cell(y-self.begin_line,x-self.begin_row,obj).alignment = Alignment(horizontal='center', vertical='center')
...
class MyThread (threading.Thread):
    ...
    def run(self):
        ...
        if type(self.func)==type(''):
            ...
            result=eval( ('self.instance.{}(*{},**{})').format(self.func,self.arg,self.kw) )
...
def draw_points(*ps,img='D:\\Desktop\\.py/block.jpg',resize=(900,900)):
    ...
    for n,p in enumerate(ps):
        ...
        # print((p1[0]+6,p[1]-6))
        cv2.putText(img,'p'+str(n+1),(p1[0]+6,p1[1]-6),cv2.FONT_HERSHEY_TRIPLEX,0.4,bgr2rgb((255,0,0)),1)
...
def pic_insert_pic(
    ...
    if back_resize_size:    #变化背景大小
        pic_back=cv2.resize(pic_back,back_resize_size,interpolation=cv2.INTER_NEAREST)
    ...
    if random_size: #随机变化大小
        ...
        pic_front=cv2.resize(pic_front,(x2,y2),interpolation=cv2.INTER_NEAREST)
        fx,fy = pic_front.shape[0],pic_front.shape[1]
        
   
    if resize and random_size==False:   #按输入变化大小
        if resize_mode==1:      
            pic_front=cv2.resize(pic_front,front_resize_size,interpolation=cv2.INTER_NEAREST)
    ...
    if random_position: #随机插入位置
        if insert_position != None:
            print('Warning: in mode random_position,defined insert_position would not be used')
        x=random.randint(0,bx-fx)
        y=random.randint(0,by-fy)
        insert_position=(x,y)
    
    pic_back[insert_position[0]:insert_position[0]+fx, insert_position[1]:insert_position[1]+fy] = pic_front
    new_pic=pic_back
    if output_form=='path':
...
def renames(path,new=None,nokey=None):
    ...
    for i, file in enumerate(files):
        if nokey not in file:
            NewName = os.path.join(path, str(a)+(new if new else filename2end(file)))
            newn.append(NewName)
...
if __name__=='pysl':
    # current_time = getime()
    # pysl_including=True
    # print('当前目录: {} \n当前时间: {}'.format(os.getcwd(),current_time ))
    # print('*************************\n>> PYSL Module Importing <<\n')
    pysl_including=True
    current_time = getime()
    ...
    try:
        ...
        raise Exception(cfmt_print('@^Rb@Module pysl did not update',return_=True))
